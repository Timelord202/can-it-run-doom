...
export default async function ShowDevices({ devices }: { devices: Device[] }) {
  const deviceStats = devices?.map((device) => {
    const cpuPass = device.clockspeed >= DOOM_REQUIREMENTS.clockspeed.value;
    const ramPass = device.ram >= DOOM_REQUIREMENTS.ram.value;
    const storagePass = device.storage >= DOOM_REQUIREMENTS.storage.value;
  ...
  return (
    <main className="min-h-screen bg-zinc-950 px-6 py-12 text-white">
      <div className="mx-auto max-w-4xl">
        <Link
          href="/"
          className="text-zinc-400 transition hover:text-white flex items-center gap-2 mb-5"
        >
          <IoChevronBack />
        ...
        <div className="space-y-4">{deviceStats}</div>
      </div>
    </main>
  );
}
